...
class LeaseVectorStore:
    ...
    def chunk_text(self, text: str, chunk_size: int = 200, overlap: int = 30) -> List[str]:
        ...
        logger.info(f"✅ Created {len(final_chunks)} chunks (avg {sum(len(c.split()) for c in final_chunks) // len(final_chunks) if final_chunks else 0} words each)")
        return final_chunks
    ...
    def _extract_keywords(self, text: str) -> List[str]:
        ...
        stop_words = {
            ...
            'can', 'this', 'that', 'these', 'those', 'what', 'where', 'when', 'why',
        ...
        if not results:
            ...
            if any(word in query.lower() for word in ['rent', 'payment', 'cost', 'monthly', 'deposit', 'fee']):
                for idx, chunk in enumerate(self.chunks):
                    ...
                    if (any(symbol in chunk['text'] for symbol in ['$', '£', '€', '¥']) or 
                        any(word in text_lower for word in ['rent', 'payment', 'monthly', 'cost', 'deposit', 'fee', 'dollar', 'pound']) or
                        # Handle OCR errors in common financial terms
                        any(word in text_lower for word in ['r3nt', 'r€nt', 'm0nthly', 'c0st', 'd0llar']) or
    ...
    def _calculate_similarity(self, query_keywords: List[str], chunk: Dict) -> float:
        ...
        if any(word in query_keywords for word in ['rent', 'payment', 'monthly', 'cost', 'deposit', 'fee']):
            if any(symbol in chunk['text'] for symbol in ['$', '£']) or any(word in chunk_text_lower for word in ['rent', 'payment', 'monthly', 'cost', 'deposit', 'fee']):
        ...
        if any(word in query_keywords for word in pet_query_words):
            if any(word in chunk_text_lower for word in ['pet', 'pets', 'animal', 'animals', 'dog', 'dogs', 'cat', 'cats', 'breed', 'vaccination']):
                normalized_score += 0.4
        ...
        if any(word in query_keywords for word in utility_query_words):
            if any(word in chunk_text_lower for word in ['utility', 'utilities', 'electric', 'electricity', 'gas', 'water', 'sewer', 'internet']):
                normalized_score += 0.4
